 ...
 * Live photo capture using getUserMedia API with Fallback & Demo mode
 */
...
const Camera = {
  ...
  /**
   * Initialize camera and show preview in a modal
   * @param {string} mode - 'checkin' or 'checkout'
   * @param {Function} onCapture - callback with base64 photo data
   */
  async open(mode, onCapture) {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      ...
      console.warn('getUserMedia API is not supported or context is non-secure');
  ...
  capture() {
    if (!this.videoEl || this.videoEl.videoWidth === 0) {
      // Fallback if camera video isn't ready
      const demoData = this.generateDemoPhoto();
      this.showCapturedImage(demoData);
      return;
    }
    ...
    // Draw video frame horizontally mirrored (to match live camera preview)
    ctx.save();
